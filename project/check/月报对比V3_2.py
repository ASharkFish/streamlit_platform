...
def main(inp_,inp_date):
    ...
    use_df_ = QC_DF[QC_DF['客户'].isin(inp_)].reset_index(drop=True)
    ...
    for i in tqdm(range(len(use_df_))):
        if '21' in use_df_.loc[i,'客户']:
            ...
            server='192.168.0.15'
        lg(i,server)
        with pd.ExcelWriter(f'{result_path}/{inp_[0]}等月报对比_结果.xlsx') as mn_writer:
            mer_df.merge(pd.concat(a_box,axis=0)[['月份','数据库名','备份数据库名','平台','新数据库销售额','新数据库升销量','备份数据库销售额','备份数据库升销量','销售额差异','升销量差异','是否差异']],how='right', on=['数据库名']).drop_duplicates().reset_index(drop=True).to_excel(mn_writer,sheet_name='平台',na_rep='',index=False)
            mer_df.merge(pd.concat(b_box,axis=0)[['月份','数据库名','备份数据库名','平台','品牌','新数据库销售额','新数据库升销量','备份数据库销售额','备份数据库升销量','销售额差异','升销量差异','是否差异']],how='right', on=['数据库名']).drop_duplicates().reset_index(drop=True).to_excel(mn_writer,sheet_name='平台 品牌',na_rep='',index=False)
            try:
                mer_df.merge(pd.concat(c_box,axis=0)[['月份','数据库名','备份数据库名','平台','品牌','产品名称','新数据库销售额','新数据库升销量','备份数据库销售额','备份数据库升销量','销售额差异','升销量差异','是否差异']],how='right', on=['数据库名']).drop_duplicates().reset_index(drop=True).to_excel(mn_writer,sheet_name='平台 品牌 产品名称',na_rep='',index=False)
            except:
                pass
