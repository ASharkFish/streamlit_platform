...
def sql_directory():
    ...
    sql = '''select CAST ( 客户 AS nvarchar ( 500 ) ),CAST ( 品类 AS nvarchar ( 500 ) ),CAST ( 客户品类 AS nvarchar ( 500 ) ),CAST ( 数据库名 AS nvarchar ( 500 ) ),\
...
def check_21(df_whole,time1,time2,kehu_name):
    ...
    df_list=[]
    for num1 in tqdm(range(len(df_input))):
...
def main(time_begin,time_end,kehu_list):
    ...
    error=[]
    write=pd.ExcelWriter(f'{result_path}/{kehu_list[0]}等一对多输出.xlsx')

    for kehu_name in kehu_list:
        
        if '弦镜' in kehu_name:
            dfresult=check_21(df_directory,time_begin,time_end,kehu_name)
            dfresult.to_excel(write,sheet_name=kehu_name,index=False)
        else:
...
if __name__ == "__main__":
    ...
    main(time_begin,time_end,kehu_list)

    # df_json = dfresult.to_json(orient='records')
    # print(dfresult)
