...
def main(time1,time2):    
    ...
    #数据接口
    df_whole=数据查询.main('轮胎',time2,time1)
    ...
    # df_url,df_brand=sql_check() 
    df_shop=sql_shop()
    df_category=sql_category()
    write=pd.ExcelWriter(f'{result_path}/模型推算_轮胎{time2}.xlsx')
    df_result=[]
    ...
    dftest_pinglun=dftest.copy()
    dftest_pinglun=dftest_pinglun.rename(columns={'评论数':'本月评论数'})
    dftrain=pd.merge(dftrain,dftest_pinglun[['url','本月评论数']],how='left',on=['url'])
    dftrain['评论比例']=dftrain['本月评论数']/dftrain['评论数']
    dftrain=dftrain.fillna(0)
    dftrain['评论数']=dftrain.apply(lambda row: row['评论数'] if (row['评论比例']<3) else( row['评论数']*random.uniform(1.5,2)  if (3<row['评论比例']<5) else( row['评论数']*random.uniform(2.5,3) if (5<row['评论比例']<10) else( row['评论数']*random.uniform(5,6) ))),axis=1  )
    ...
    df_train['前一月销评比'] = df_train.apply(lambda row: np.nan if (row['评论数'] == 0) else (row['销量']/row['评论数']), axis=1)
    df_train=df_train.rename(columns={'评论数':'前一月评论数','销量':'前一月销量','真实评论条数':'前一月真实评论条数'})
    #结果集
    #关联外部url销量
    # dftest=pd.merge(dftest,df_url[['url','外部销量']],how='left',on=['url']).reset_index(drop=True)
    dftest['外部销量']=0

    #关联3个销评比及预测评论数
    dftest=pd.merge(dftest,df_train[['url','url销评比','前一月评论数','前一月销量','前一月销评比','前一月真实评论条数']],how='left',on=['url'])
    ...
    dftest['销评比'] = dftest.apply(lambda row: 0 if  (row['url销评比']==0)&(row['品类id销评比']==0)   else random.uniform(3.5,4.5) if ((row['url销评比']<2)|(row['url销评比']==np.inf)) else (row['url销评比']*random.uniform(1.9,2.1) if ((row['url销评比']>=2)&(row['url销评比']<=3)) else row['url销评比']  )  ,axis=1) 
    min_value = dftest['销评比'].loc[dftest['销评比'] > 100].min()
    max_value = dftest['销评比'].loc[dftest['销评比'] > 100].max()

    dftest.loc[dftest['销评比'] > 100, '销评比'] = 25 + ((dftest['销评比'] - min_value) / (max_value - min_value)) * 75
    ...
    # dftest_group=dftest[['品牌id','计算销售额']].groupby(['品牌id'],as_index=False).sum()
    #计算训练部分品牌销售额
    ...
    #输出
    df_test=dftest[['时间','品类id','二级品类名称','品牌id','品牌','店铺id','店铺名称','店铺类型','url','销评比','url销评比','品牌id销评比','品类id销评比', '预测销量','最终销量','调整系数','调整销量','前一月销量','评论数','真实评论条数','前一月评论数','前一月真实评论条数','前一月销评比','平均成交价','榜单价格','榜单销量','销售信息','榜单','榜单日期']].drop_duplicates().reset_index(drop=True)
    df_result.append(df_test)
    df_test.to_excel(write,sheet_name=f'url统计量{time1}',index=False)
    dftest_group.to_excel(write,sheet_name=f'品类品牌增速{time1}',index=False)
