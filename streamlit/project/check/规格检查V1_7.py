...
def sql_whole():
    sql='''
        select
        CAST(客户 AS nvarchar ( 2000 )),CAST(品类 AS nvarchar ( 2000 )),CAST(数据库名 AS nvarchar ( 2000 )),CAST(单规格是否带单位 AS nvarchar(2000)),\
        CAST(产品名称字段名 AS nvarchar ( 2000 )),CAST(单规格字段名 AS nvarchar ( 2000 )),CAST(套装数字段名 AS nvarchar ( 2000 )),CAST(总规格字段名	AS nvarchar ( 2000 )),\
        CAST(制造商字段名 AS nvarchar ( 2000 )),CAST(客户品类 AS nvarchar ( 2000 )),CAST(时间 AS nvarchar ( 2000 )) from [QC].[dbo].[产品名称规格套装数判断]

    '''
    print(sql)
    database='QC'
    x=[ '客户','品类','数据库名','单规格是否带单位','产品名称字段名','单规格字段名','套装数字段名','总规格字段名','制造商字段名','客户品类','时间']

    df= sql_connect.select('192.168.0.15',database,'sqlsever',sql,x)
    df['时间']=df['时间'].fillna('null')
    return df
...
def main(customer,time1,time2):
    ...
    for kehu_name in tqdm(customer) :
        ...
        write=pd.ExcelWriter(f'{result_path}/{kehu_name}规格检查.xlsx')
        df_wrong.to_excel(write,sheet_name=f'抛出')
        df_result.to_excel(write,sheet_name=f'产品名称错误')
        write.close()
    print('\n'.join(error))
    print('输出完成！')
