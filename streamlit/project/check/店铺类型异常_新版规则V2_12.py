...
def judement_check(df,rule,df_self_support,independent_rules,kehuname,df_shop_info):
    ...
    if kehuname in spicel_shop_info:
        ...
        for key,group in tqdm(df_group):
            platform_name=key[1]
            group=group.reset_index(drop=True)
...
def main(customer,time_start,time_end):
    ...
    filename=f'{result_path}/{customer[0]}等{len(customer)}个客户店铺类型异常.xlsx'
    write=pd.ExcelWriter(filename)
    

    for kehuname in tqdm(customer) :
        ...
        if wrong.empty:
            wrong.to_excel(write,sheet_name=f'{kehuname}对照',index=False)
            print(f'{kehuname}无错误')
        else:
            ...
            grouped_df.to_excel(write,sheet_name=f'{kehuname}',index=False)
            wrong.to_excel(write,sheet_name=f'{kehuname}对照',index=False)
...
if __name__ == "__main__":
    ...
    main(kehu_list,time_start,time_end)