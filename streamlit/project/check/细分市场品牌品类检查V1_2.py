...
def main(customer,time_start,time_end):
    ...
    for kehu in customer:
        wrong1_list=[]
        wrong2_list=[]
        write=pd.ExcelWriter(f'{result_path}/{kehu}细分市场品牌品类检查.xlsx')
        ...
        result_wrong2.to_excel(write,sheet_name='匹配错误',index=False)
        result_wrong1.to_excel(write,sheet_name='未匹配',index=False)
        write.close()
